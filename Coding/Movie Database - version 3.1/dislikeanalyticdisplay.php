...
try{	
		...
		$query = $conn->prepare("SELECT title,dislikes FROM movies_table WHERE dislikes >= 1 ORDER BY dislikes DESC LIMIT 10");
		// executing query
		$query->execute();
		...
		$result = $query->fetchAll(PDO::FETCH_OBJ);
			foreach($result as $row) {
				...
				array_push($array, array("label"=> $row->title, "y"=> $row->dislikes));
			}
			$conn = null;
...
echo "Sorry for not liking this movie!";
?>
<br>
<?php
