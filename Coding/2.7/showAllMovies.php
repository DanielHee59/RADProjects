<?php
include "index.php";//includ homepage layout
?>

	<h1> All Movies </h1>
	<?php
	if($_SERVER["REQUEST_METHOD"] == "POST"){ //if button pressed
	}
		require('connect.php');// create connection
		//sql query to display all movies
		$result = mysqli_query($conn, "SELECT * FROM movies_table");
		if(mysqli_num_rows($result) > 0) //if contains one result or more
		{
			//create table heading of every column
